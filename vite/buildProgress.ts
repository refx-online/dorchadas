		...
		config(config, { command }) {
			if (command === 'build') {
				...
				progressBar = new SingleBar({
					format: `${chalk.blue('Building source...')} ${chalk.gray('|')} ${chalk.red.bold(
						'{bar}'
					)} ${chalk.blue(`{percentage}%`)} ${chalk.gray('|')} ${chalk.blue(
						`{chunks}/{totalChunks} Chunks`
					)} ${chalk.gray('|')} ${chalk.blue(`{transformed}/{totalTransformed} Transformed`)}`,
					barCompleteChar: `\u2501`,
					barIncompleteChar: `\u2501`,
					barGlue: '\x1b[0m',
					hideCursor: true
