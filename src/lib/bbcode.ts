export function parseBBCodeToHtml(bbcode: string): string {
	...
	const conversions: [RegExp, string | ((...args: string[]) => string)][] = [
		...
		[
			/\[quote="(.*?)"\]([\s\S]*?)\[\/quote\]/g,
			'<blockquote class="userpage-quote"><strong>$1 said:</strong><br>$2</blockquote>'
		],
		[/\[quote\]([\s\S]*?)\[\/quote\]/g, '<blockquote class="userpage-quote">$1</blockquote>'],

		[
			/\[box=(.*?)\]([\s\S]*?)\[\/box\]/g,
			'<div class="userpage-box" style="background-color:$1;">$2</div>'
		],
		...
		[
			/\[spotify(?:=(\d+),(\d+))?\](https?:\/\/open\.spotify\.com\/[^\s\]]+)\[\/spotify\]/g,
			(match, width, height, url) => {
