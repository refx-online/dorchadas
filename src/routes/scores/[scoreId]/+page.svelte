<script lang="ts">
	import { appName, appUrl, getReplayUrl, avatarUrl } from '$lib/env';
	...
	const getGradeClass = (grade: string) => {
		switch (grade) {
			case 'SSH':
			case 'SS':
				return 'text-gray-100 drop-shadow-[0_0_20px_rgba(255,255,255,0.8)] animate-pulse';
			case 'SH':
			case 'S':
				return 'text-gray-200 drop-shadow-[0_0_15px_rgba(229,231,235,0.6)]';
			case 'A':
				return 'text-gray-300 drop-shadow-[0_0_10px_rgba(209,213,219,0.5)]';
			case 'B':
				return 'text-gray-400 drop-shadow-[0_0_8px_rgba(156,163,175,0.4)]';
			case 'C':
				return 'text-gray-500 drop-shadow-[0_0_6px_rgba(107,114,128,0.3)]';
			case 'D':
				return 'text-gray-600 drop-shadow-[0_0_4px_rgba(75,85,99,0.2)]';
			default:
...
<svelte:head>
	{#if data.score && data.beatmap}
		<title
			>{appName} :: {data.player?.name} - {data.beatmap.artist} - {data.beatmap.title} [{data
				.beatmap.version}] - score info</title
		>
		<meta property="og:type" content="profile" />
		<meta
			property="og:title"
			content="{data.player?.name} - {data.beatmap.artist} - {data.beatmap.title} [{data.beatmap
				.version}] - score info"
		/>
		...
		<div class="mx-auto max-w-6xl relative z-10 glass-depth rounded-lg overflow-hidden">
			<div class="relative bg-black/45 rounded-t-lg overflow-hidden">
				<div class="relative z-10 p-4 sm:p-6">
					...
					<h1
						class="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-white mb-2 leading-tight"
					>
						<a
			...
			<div class="relative overflow-hidden z-10">
				<div
					...
					style="background-image: url('https://assets.ppy.sh/beatmaps/{data.beatmap
					...
					<div class="flex flex-col items-center gap-6 lg:gap-8">
						<div class="flex flex-col lg:flex-row items-center w-full gap-6 lg:gap-8">
							<div class="flex flex-col items-center gap-4 flex-shrink-0">
								<div
									class="relative backdrop-blur-sm bg-black/30 rounded-full p-3 sm:p-4 shadow-2xl border border-gray-600/30"
								>
									<div
										class="absolute inset-0 rounded-full bg-gradient-to-br from-gray-400/10 to-gray-600/10 animate-pulse"
									></div>
									<svg
										class="w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48 transform -rotate-90 relative z-10 drop-shadow-lg"
										viewBox="0 0 100 100"
									>
										<defs>
									...
									<div class="absolute inset-0 flex items-center justify-center">
										<div class="relative">
											<span
												class="text-4xl sm:text-5xl lg:text-6xl font-bold {getGradeClass(
													data.score.grade
												)} relative z-10 tracking-wider transform hover:scale-110 transition-transform duration-300"
											>
												{data.score.grade.replace('H', '')}
											</span>
											<div
												class="absolute inset-0 text-4xl sm:text-5xl lg:text-6xl font-bold text-white/20 blur-sm -z-10"
											>
												{data.score.grade.replace('H', '')}
											</div>
										</div>
								...
								{#if data.score.mods > 0}
									<div
										class="flex flex-wrap justify-center gap-1 sm:gap-2 backdrop-blur-sm bg-black/30 rounded-lg p-2 sm:p-3 border border-gray-600/20 shadow-lg items-center"
									>
										{#each parseModsInt(data.score.mods, data.score.mods_json, data.score.clock_rate) as mod}
							...
							<div class="flex-1 text-center lg:text-left w-full min-w-0">
								<div
									class="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-3 sm:mb-4 break-all"
								>
									{formatNumber(data.score.score)}
								</div>
								<div
									class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-2 sm:gap-4 text-base sm:text-lg"
								>
						...
						<div
							class="w-full backdrop-blur-sm bg-black/30 rounded-lg p-4 sm:p-6 lg:absolute lg:right-0 lg:top-0 lg:w-auto lg:mt-16 border border-gray-600/20 shadow-2xl"
						>
							<div class="grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8 text-center mb-4 sm:mb-6">
								<div class="transform hover:scale-105 transition-transform duration-200">
									...
									<div
										class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-200 drop-shadow-lg"
									>
										{formatAccuracy(data.score.acc)}
									</div>
								</div>
								<div class="transform hover:scale-105 transition-transform duration-200">
									<div class="text-xs text-gray-400 uppercase tracking-wide mb-1">Max Combo</div>
									<div
										class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-300 drop-shadow-lg"
									>
										{formatNumber(data.score.max_combo)}x
									</div>
								</div>
								<div class="transform hover:scale-105 transition-transform duration-200">
									<div class="text-xs text-gray-400 uppercase tracking-wide mb-1">PP</div>
									<div
										class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-100 drop-shadow-lg"
									>
										{Math.round(data.score.pp)}
									</div>
								</div>
							</div>
							...
							<div class="grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8 text-center">
								<div class="transform hover:scale-105 transition-transform duration-200">
									...
									<div
										class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-300 drop-shadow-md"
									>
										{formatNumber(data.score.n100)}
									</div>
								</div>
								<div class="transform hover:scale-105 transition-transform duration-200">
									<div class="text-xs text-gray-400 uppercase tracking-wide mb-1">Meh</div>
									<div
										class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-400 drop-shadow-md"
									>
										{formatNumber(data.score.n50)}
									</div>
								</div>
								<div class="transform hover:scale-105 transition-transform duration-200">
									<div class="text-xs text-gray-400 uppercase tracking-wide mb-1">Miss</div>
									<div
										class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-500 drop-shadow-md"
									>
										{formatNumber(data.score.nmiss)}
									</div>
								</div>
							</div>
						</div>
			...
			<div class="relative bg-white/[0.04] rounded-b-lg overflow-hidden">
				...
				>
					<div class="flex items-center gap-3 sm:gap-4">
						<div
							class="w-12 h-12 sm:w-16 sm:h-16 bg-gray-600 rounded-full overflow-hidden flex-shrink-0"
						>
							<img
	...
	{:else}
		<div class="mx-auto card p-4 sm:p-6 py-8 sm:py-12 max-w-4xl">
			...
			>
				...
				<div
					class="flex flex-col items-center lg:items-start justify-normal lg:ps-20 gap-2 w-full lg:w-auto"
				>
					<p
						class="text-base sm:text-lg font-semibold underline underline-offset-2 text-center lg:text-left"
					>
					...
					<a
						class="mx-auto mt-6 sm:mt-10 btn variant-filled-surface text-sm sm:text-base px-4 sm:px-6 py-2"
						href="/"
					>
