...
const sendDiscordWebhookRank = async (beatmap: any, user: any, newStatus: RankedStatus) => {
	const webhookUrl = env.DISCORD_WEBHOOK_RANK;
	if (!webhookUrl) {
		return;
	}
...
export const POST: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		if (!redis) {
			throw error(500, 'Redis connection failed');
		}
		...
		await sendDiscordWebhookRank(updatedBeatmap, sessionUser, newStatus);
